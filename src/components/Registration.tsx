                      ...
                      <div
                        ...
                        className={`rounded-[32px] p-8 flex flex-col justify-between relative transition-all duration-300 select-none overflow-hidden ${
                          isActive
                            ...
                            : isClosed
                            ? "bg-[#090A1A] text-gray-400 border border-red-500/10 opacity-70 cursor-not-allowed"
                            : "bg-[#090A1A] text-gray-100 border border-white/5 opacity-70 cursor-not-allowed"
                        }`}
                      >
                        {/* Ribbon "OUVERT" or "CLÔTURÉ" on the top right */}
                        {isActive && (
                        ...
                        {isClosed && (
                          <div className="absolute top-0 right-0 bg-red-950/80 text-red-300 border-b border-l border-red-500/30 font-sans font-black text-[9px] tracking-widest uppercase px-4 py-1.5 rounded-bl-xl">
                            CLÔTURÉ
                          </div>
                        )}
                        ...
                        <div className="mt-8 pt-4">
                          {isActive ? (
                            ...
                            <button
                              type="button"
                              disabled
                              className={`w-full py-3.5 px-6 rounded-2xl font-sans font-black text-[11px] tracking-widest uppercase transition-all duration-300 text-center cursor-not-allowed ${
                                isClosed
                                  ? "bg-red-950/30 text-red-400/60 border border-red-500/20"
                                  : "bg-[#14152A] text-gray-500 border border-white/5"
                              }`}
                            >
                              <span>{isClosed ? "OFFRE CLÔTURÉE" : "BIENTÔT DISPONIBLE"}</span>
                            </button>
                          )}
                          
                          {/* Message de statut en bas */}
                          <span className="block text-center text-[10px] text-gray-500 font-sans mt-2.5">
                            {isActive ? "Redirection sécurisée" : isClosed ? "Inscriptions fermées" : "Ouverture le 24 août"}
                          </span>
                        </div>
                      </div>
                    );

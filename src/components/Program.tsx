...
import { Clock, MapPin, Filter, ChevronDown, Calendar, Sparkles } from "lucide-react";
    ...
    <section id="program" className="py-20 md:py-28 bg-[#0F1026] text-white scroll-mt-12 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-8 mb-12">
          <div className="max-w-xl text-left space-y-4">
            <div className="inline-flex items-center space-x-2 bg-amber-400/10 border border-amber-400/20 px-3 py-1 rounded-full w-fit">
              ...
              <span className="text-xs font-mono tracking-wider uppercase font-black text-amber-400">
                L&apos;AGENDA OPTIMISÉ
              </span>
            </div>
          ...
          {filteredSessions.length > 0 && (
            <button
              onClick={toggleAll}
              className="text-xs font-semibold tracking-wide text-amber-400 hover:text-white transition-colors underline underline-offset-4 cursor-pointer self-start lg:self-end"
            >
        ...
        <div className="bg-[#151631]/80 border border-white/5 rounded-3xl p-4 sm:p-6 mb-10 max-w-5xl mx-auto flex flex-col md:flex-row items-stretch md:items-center justify-between gap-6">
          ...
          {/* Day selection tabs */}
          <div className="flex bg-[#0F1026]/90 p-1.5 rounded-2xl border border-white/5 items-center space-x-1.5 self-start md:self-auto w-full md:w-auto">
            <button
              onClick={() => setActiveDay(1)}
              className={`flex-1 md:flex-none px-4 sm:px-6 py-2.5 rounded-xl font-sans text-xs sm:text-sm font-black tracking-wide uppercase transition-all duration-300 cursor-pointer flex items-center justify-center space-x-2 relative ${
                activeDay === 1
                  ? "bg-amber-400 text-[#0F1026] shadow-md font-extrabold"
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 flex-grow w-full">
                        {/* Compact Clock Badge */}
                        <div className="flex items-center space-x-2 bg-amber-400/5 px-3.5 py-1.5 rounded-xl border border-amber-400/10 text-amber-400 font-mono text-xs sm:text-sm font-bold flex-shrink-0">
                        ...
                        <div className="space-y-1.5 flex-grow text-left">
                          ...
                          <h3 className="text-base sm:text-lg font-sans font-black text-white leading-snug tracking-wide group-hover:text-amber-400 transition-colors">
                            {session.title}
                          </h3>
                        </div>
                      </div>
                      ...
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all flex-shrink-0 ${
                        isExpanded ? "bg-[#00A6DF]/15 text-white" : "bg-white/5 text-gray-400 group-hover:text-amber-400 group-hover:bg-amber-400/15"
                      }`}>
                          ...
                          <div className="px-5 pb-6 sm:px-6 sm:pb-8 pt-2 border-t border-white/5 space-y-5">
                            ...
                            {speaker && (
                              <div className="p-4 rounded-2xl bg-[#0F1026]/60 border border-white/5 flex flex-col sm:flex-row items-start sm:items-center gap-4 hover:border-white/10 transition-colors max-w-2xl text-left">
                                <div className="h-12 w-12 rounded-full overflow-hidden border-2 border-amber-400/20 flex-shrink-0 bg-slate-900 shadow-md">
                                  <img
                                    src={speaker.image}
                                    alt={speaker.name}
                                ...
                                <div className="space-y-1">
                                  <span className="block text-[9px] font-mono font-black text-amber-400 uppercase tracking-widest">
